...
func TestClientPPPDefaultCommandLine(t *testing.T) {
	...
	// The MTU is the measured link ceiling less the multilink header; the MRU is
	// the ceiling itself, because what arrives is not bundled.
	mru := argInt(t, inv.args, "mru")
	mtu := argInt(t, inv.args, "mtu")
	assert.Equal(t, mru-pppbridge.MLPPPOverhead, mtu,
		"multilink costs four octets on the way out and none on the way in")
	assert.Equal(t, pppbridge.AutoPPPMTU(pppbridge.MTUParams{RemoteAddr: testRemote()}), mru)
}
...
func TestClientPPPSSTPCommandLine(t *testing.T) {
	...
	assert.Equal(t, argValue(t, inv.args, "mru"), argValue(t, inv.args, "mtu"))
}
