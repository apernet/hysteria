...
func (h *L2TPPPPHandler) HandlePPP(
	...
	h.Logger.Debug("PPP request accepted, starting LCP negotiation",
		zap.String("addr", addr.String()),
		zap.String("id", id))
	...
	// Send auth result to client so it can proceed to NCP phase.
	// The LNS receives proxy auth via ICCN and verifies independently.
	...
	go func() {
		...
		for {
			...
			for {
				...
				if ce := h.Logger.Check(zap.DebugLevel, "bridge: client -> LNS"); ce != nil {
					ce.Write(zap.String("id", id),
						zap.Uint16("proto", proto),
						zap.Int("len", len(payload)),
						zap.String("hex", hexHead(rawPPP)))
				}
	...
	go func() {
		for {
			...
			if ce := h.Logger.Check(zap.DebugLevel, "bridge: LNS -> client"); ce != nil {
				ce.Write(zap.String("id", id),
					zap.Uint16("proto", proto),
					zap.Int("len", len(payload)),
					zap.String("hex", hexHead(rawPPP)))
