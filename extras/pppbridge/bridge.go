...
func (b *Bridge) Run(ctx context.Context, dialFn DialFn) (runErr error) {
	...
	for {
		...
		control, data, cf, err := b.dialWithRetry(ctx, dialFn, childDone)
		if err != nil {
			return err
		}
		closeFn = cf
		...
		for {
			...
			// Session died but child is still alive -- proactively re-dial
			// instead of waiting for the child to send a new LCP Config-Request.
			b.Logger.Info("session lost, proactively re-dialing")
			control, data, cf, err = b.dialWithRetry(ctx, dialFn, childDone)
			if err != nil {
				return err
			}
			closeFn = cf
			sess = b.startSession(control, data, childWriter, &writeMu, txFrame{}, childDone)
		}
	}
}
...
func (b *Bridge) startSession(
	...
	if len(initial.hdlcFrame) > 0 || len(initial.rawPPP) > 0 {
