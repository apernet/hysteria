...
func BuildICCN(info *ProxyInfo) []byte {
	...
	// Proxy LCP CONFREQ AVPs: Cisco IOS expects only the LCP options
	// (not the full Code+ID+Length+Options packet). Strip the 4-byte
	// LCP header before encoding.
	if len(info.InitialReceivedCONFREQ) > 4 {
		buf = append(buf, EncodeBytesAVPWithFlags(AVPInitialReceivedLCPCONFREQ, info.InitialReceivedCONFREQ[4:], false)...)
	}
	if len(info.LastSentCONFREQ) > 4 {
		buf = append(buf, EncodeBytesAVPWithFlags(AVPLastSentLCPCONFREQ, info.LastSentCONFREQ[4:], false)...)
	}
	if len(info.LastReceivedCONFREQ) > 4 {
		buf = append(buf, EncodeBytesAVPWithFlags(AVPLastReceivedLCPCONFREQ, info.LastReceivedCONFREQ[4:], false)...)
	}

	// Proxy Authentication AVPs
	buf = append(buf, EncodeUint16AVPWithFlags(AVPProxyAuthenType, info.AuthType, false)...)
	buf = append(buf, EncodeStringAVPWithFlags(AVPProxyAuthenName, info.AuthName, false)...)
	// Proxy Authen ID: single byte, padded to 2 bytes with leading zero per RFC 2661
	buf = append(buf, EncodeAVPWithFlags(AVPProxyAuthenID, []byte{0, info.AuthID}, false)...)
	if len(info.AuthChallenge) > 0 {
		buf = append(buf, EncodeBytesAVPWithFlags(AVPProxyAuthenChallenge, info.AuthChallenge, false)...)
	}
	if len(info.AuthResponse) > 0 {
		buf = append(buf, EncodeBytesAVPWithFlags(AVPProxyAuthenResponse, info.AuthResponse, false)...)
	}
